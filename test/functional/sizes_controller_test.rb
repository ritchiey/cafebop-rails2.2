...
class SizesControllerTest < ActionController::TestCase
  ...
  def self.should_display_new_page
    should_assign_to(:size, :class=>Size)
    should_respond_with :success
    should_render_with_layout
  ...
  def self.when_creating
   context "when creating a size" do
     context "with valid data" do
       setup do
         post :create, :menu_item_id=>@menu_item.id, :size=>@valid.merge(:name=>'Porcupine')
       end
       context "" do
         yield
       end
  ...
  def self.when_updating_with_valid_data
    context "when updating with valid data" do
      setup do
        put :update, :id=>@size.id, :size=>{:name=>'Willow'}
      end
      context "" do
        yield
      end
  ...
  context "Given a shop with a menu with one menu_item with a size" do
    setup do
      @shop = Shop.make
      @menu = @shop.menus.make
      @menu_item = @menu.menu_items.make
      @size = @menu_item.sizes.make
      @valid = @menu_item.sizes.make_unsaved.attributes
    end
